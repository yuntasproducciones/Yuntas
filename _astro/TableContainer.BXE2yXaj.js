...
`),t=new Blob(["\uFEFF"+i],{type:"text/csv;charset=utf-8;"}),n=document.createElement("a"),l=URL.createObjectURL(t);n.setAttribute("href",l),n.setAttribute("download",`blogs_export_${new Date().toISOString().split("T")[0]}.csv`),n.style.display="none",document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(l),console.log("✅ CSV exportado exitosamente"),alert(`✅ Se exportaron ${e.length} registros a CSV`)}catch(r){console.error("❌ Error al exportar CSV:",r),alert("❌ Error al exportar a CSV")}},y=e=>{try{if(!e||e.length===0){alert("❌ No hay datos para exportar");return}const r=window.open("","_blank");if(!r){alert("❌ Por favor permite ventanas emergentes para generar el PDF");return}const i=`
        ...
        <body>
          <div class="header">
            ...
            <div class="stats">
              ...
              <div class="stat-item">
                <div class="stat-number">${e.reduce((t,n)=>t+(n.parrafos?.length||0),0)}</div>
                <div class="stat-label">Total Párrafos</div>
              </div>
              <div class="stat-item">
                <div class="stat-number">${e.reduce((t,n)=>t+(n.imagenes?.length||0),0)}</div>
    ...
    `;r.document.write(i),r.document.close(),r.onload=()=>{setTimeout(()=>{r.focus(),r.print(),r.close()},250)},console.log("✅ Tabla enviada a imprimir")}catch(r){console.error("❌ Error al imprimir:",r),alert("❌ Error al imprimir")}};function S({children:e,tableType:r="default",headerContent:i,exportData:t=[],onAddNew:n}){const l=()=>{if(t.length===0){alert("No hay datos para exportar");return}b(t)},o=()=>{if(t.length===0){alert("No hay datos para exportar");return}f(t)},c=()=>{if(t.length===0){alert("No hay datos para exportar");return}y(t)},s=()=>{if(t.length===0){alert("No hay datos para imprimir");return}v(t)},u=()=>{if(i)return i;switch(r){case"seguimiento":return a.jsxs("div",{className:"flex flex-wrap gap-2 mb-4",children:[a.jsx("button",{className:"bg-sky-600 text-white px-4 py-2 rounded-xl font-semibold opacity-80 hover:opacity-100 transition-opacity",children:"MENSAJES"}),a.jsx("button",{className:"bg-sky-600 text-white px-4 py-2 rounded-xl font-semibold opacity-80 hover:opacity-100 transition-opacity",children:"MEDIO DE SEGUIMIENTO"}),a.jsx("button",{className:"bg-sky-600 text-white px-4 py-2 rounded-xl font-semibold opacity-80 hover:opacity-100 transition-opacity",children:"MONITOREO"})]});case"blogs":case"productos":case"usuarios":return a.jsxs("div",{className:"flex flex-wrap gap-2 mb-4",children:[n&&a.jsx("button",{onClick:n,className:"bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl font-semibold transition-colors",children:"PUBLICAR"}),a.jsx("button",{onClick:l,className:"bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-xl font-semibold transition-colors",children:"EXPORTAR A CSV"}),a.jsx("button",{onClick:o,className:"bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-xl font-semibold transition-colors",children:"EXPORTAR A EXCEL"}),a.jsx("button",{onClick:c,className:"bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-xl font-semibold transition-colors",children:"EXPORTAR A PDF"}),a.jsx("button",{onClick:s,className:"bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-xl font-semibold transition-colors",children:"IMPRIMIR"})]});default:return a.jsx("div",{className:"flex flex-wrap gap-2 mb-4",children:a.jsx("button",{className:"bg-gray-600 text-white px-4 py-2 rounded-xl hover:bg-gray-700 transition-colors",children:"ACCIÓN GENÉRICA"})})}},m=[],h=[];return(Array.isArray(e)?e:[e]).forEach(d=>{x.isValidElement(d)&&["thead","tbody","tfoot","tr"].includes(d.type||"")?m.push(d):h.push(d)}),a.jsxs("div",{className:"w-full",children:[u(),a.jsx("div",{className:"overflow-x-auto",children:a.jsx("table",{className:"min-w-full text-sm text-center border-separate border-spacing-x-2 border-spacing-y-2",children:m})}),h.length>0&&a.jsx("div",{className:"mt-2 space-y-2",children:h})]})}export{S as T};
